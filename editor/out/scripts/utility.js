...
function createEntry(ref, rawContent) {
    ...
    const contentParsingRegex = new RegExp(`(${tagPattern})|(${translatableTextPattern})`, "g");
    content = rawContent.replace(contentParsingRegex, (_, tagGroup, translatableTextGroup) => `${DELIMITER}${!!tagGroup
        ? TAG_PREFIX + tagGroup
        : ENGLISH_TEXT_PREFIX + translatableTextGroup}${DELIMITER}`);
    translatedContent = rawContent.replace(contentParsingRegex, (_, tagGroup, translatableTextGroup) => `${DELIMITER}${!!tagGroup
        ? TAG_PREFIX + tagGroup
        : ENGLISH_TEXT_PREFIX + "" /* no translated content yet */}${DELIMITER}`);
